...
for(let i in tempChartData){
    $("#tabsForCoinsWithGraphs").append(
        `
            <div role="tabpanel" class="tab-pane ${chartCount == 0 ? "active" : ""}" id="${i}">
                <canvas
    ...
    document.getElementById(`PL_BREAKDOWN_${i}`), 
    {
        type: 'line',
        data: {
            labels: tempLabels,
        ...
        $("#tabsForCoins").append(
            `
                <li class="nav-item" role="presentation">
                    <a href="#${coin}" class="nav-link ${totalListOfTradedCoins == 0 ? 'active' : ""}" role="tab" data-toggle="tab" aria-controls="${coin}">${coin}</a>
                </li>
            `
        );

        totalListOfTradedCoins++;
